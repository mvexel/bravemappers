...
parser = argparse.ArgumentParser(
    description='Process userstats.csv into a pretty web display.')
...
with open(args.infile, 'rb') as f:
    ...
    for step in range(delta.days, 0, -365):  # not quite a year but close enough
        ...
        snippet += "<div class=\"dates\" style=\"right:" +\
            percent + "%\">" + stamp + "&nbsp;&nbsp;</div>"
        y += 1
    with open(os.path.join(args.outpath, 'index.html'), 'w') as htmlout:
        htmlout.write("<html><head><script language=javascript type=text/javascript src=\'bravemappers.js\'></script><link rel=stylesheet href=\"bravemappers.css\" type=\"text/css\" media=screen><title>Brave Mappers of %s</title></head><body>%s<div id=\"title\">The brave mappers of %s<div id=\"sub\">The story of OpenStreetMap in %s in pretty shapes and colors. <a href=\"#\" onClick=\"popinfo('%s')\">more info</a></div></div><div id=logo></div><div id=info></div><div id=maininfo></div><div id=\"wrapper\">\n" % (args.region_name, snippet, args.region_name, args.region_name, json.dumps(usefulstats, default=dthandler).replace("\"", "&quot;")))
        ...
        for row in sr:
            ...
            if currentobjs < 20:
                continue  # FILTER OUT REALLY MINOR USERS
            ...
            if mayfly == '' and  forgotten == '' and power == '':
                creator = 'fair ' + creator
